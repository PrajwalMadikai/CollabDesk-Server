...
export class WorkspaceRepository implements workspaceInterface{
   async createSpace(name: string, ownerId: string, directories: { Did: string; Dname: string; }[], 
                userDetails: { uId: string; email: string; }[], meetingRoom: string, 
              type: string, trashId: string | null): Promise<workspaceEnity|null> 
        {
            const exists=await WorkspaceModal.findOne({_id:ownerId,name})
            if(exists)
            {
                return null
            }
            ...
            return new workspaceEnity(space.name,space.ownerId,
